...
4. Run `./gradlew test --tests "TestClassName"` to verify it passes
...
- Use constructor injection for dependencies (prefer `@RequiredArgsConstructor`)
- Follow package structure: `domain/{feature}/service/`, `domain/{feature}/repository/`
- Entity naming: `*Entity` suffix
- Repository naming: `*JpaRepository` (JPA) + `*Repository` (business logic)

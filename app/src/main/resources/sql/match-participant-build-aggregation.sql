...
WITH mp_batch AS MATERIALIZED (
    SELECT mp.match_id,
        ...
        mp.sub_perk0,
        mp.sub_perk1,
        mp.stat_perk_defense,
        mp.stat_perk_flex,
        mp.stat_perk_offense,
    ...
    WHERE mp.match_participant_id >= :startId
        AND mp.match_participant_id < :endId
        AND m.season = :season
    ...
    SELECT mp.match_id,
        ...
        array_agg(
            ...
            ORDER BY e.event_ts_ms,
                e.event_id
        ) FILTER (WHERE e.event_type = 'ITEM_PURCHASED') AS full_items,
        array_agg(
            e.skill_slot
            ORDER BY e.event_ts_ms,
                e.event_id
        ) FILTER (WHERE e.event_type = 'SKILL_LEVEL_UP') AS full_skills,
        MIN(e.event_ts_ms) FILTER (WHERE e.event_type = 'ITEM_PURCHASED') AS first_core_ts_ms
    FROM mp_batch mp
...
SELECT match_id,
    ...
    start_item_ids,
    boot_id,
    CASE WHEN full_items IS NULL THEN NULL
         ELSE ARRAY(SELECT unnest(full_items) LIMIT 3) END AS item_ids,
    ...
    CASE WHEN full_skills IS NULL THEN NULL
         ELSE ARRAY(SELECT unnest(full_skills) LIMIT 15) END AS skill_slots,
